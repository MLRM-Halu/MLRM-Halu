...
def resolve_image_path(entry, image_root):
    if 'image' in entry:
        return os.path.join(image_root, entry['image'].lstrip('/'))
    elif 'image_src' in entry:
        return os.path.join(image_root, os.path.basename(entry['image_src']))
    elif 'path' in entry:
        return os.path.join(image_root, entry['path'].lstrip('/'))
    else:
        raise ValueError("Cannot resolve image path from entry")
...
def run_benchmark(model, processor, data_path, image_root, save_path, prompt_template='default',
    ...
    for idx in tqdm(range(start, len(data))):
        entry = data[idx]
        image_file = resolve_image_path(entry, image_root)
        ...
        try:
            ...
            result = {
                "id": entry.get("pid", entry.get("id", idx)),
                "question": question,
                "ground_truth": entry.get("answer", entry.get("ground truth", "")),
                "question_type": entry.get("question_type", ""),
                "model_answer": response,
                "thinking": thinking,
                "thinking_length": thinking_len,
                "answer": answer,
                "image": entry.get('image', entry.get('image_src', entry.get('path', '')))
            }
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input', type=str, default='/data/MATH-V/data/test.jsonl', help='Input JSON/JSONL file path')
    parser.add_argument('--output', type=str, default='/data/responses/math_vision/ThinkLite-VL-7B.json', help='Output JSONL file path')
    parser.add_argument('--image_root', type=str, default='/data/MATH-V/images/', help='Root directory for images')
